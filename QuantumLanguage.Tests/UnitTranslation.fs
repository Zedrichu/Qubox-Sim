...
[<Test>]
let ``Check Q# compilation of circuit`` () =
...
[<Test>]
let ``Check Q# compilation of circuit #2`` () =
...
[<Test>]
let ``Check Q# compilation of circuit #3`` () =
...
[<Test>]
let ``Check Q# compilation of circuit #4`` () =
...
[<Test>]
let ``Check Q# compilation of circuit #5`` () =
...
let ``Check QuLang compilation of circuit - (double parsing procedure) #1`` () =
    ...
    let trans, err = Option.get ast |> translateCircuit |> parseQuLang
    Assert.AreEqual (Success, err)
    Assert.AreEqual (ast, trans)
    
    
[<Test>]
let ``Check QuLang compilation of circuit - (double parsing procedure) #2`` () =
...
[<Test>]
let ``Check QuLang compilation of circuit - (double parsing procedure) #3`` () =
...
[<Test>]
let ``Check QuLang compilation of circuit - (double parsing procedure) #4`` () =
